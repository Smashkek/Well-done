...
def add_note():
    note_name, ok = QInputDialog.getText(notes_win, 'дадати замітку',
                                         "Назва замітки")
    if ok and note_name != '':
...
def save_note():
    if list_notes.selectedItems():
        ...
        with open('notes_data,json', 'w') as file:
            json.dump(notes, file, sort_key=True, ensure_ascii=False)
        print(notes)
    else:
...
def del_note():
    if list_notes.selectedItems():
        ...
        print('Замітки для видалення  не вибрана!')
...
def add_tag():
    if list_notes.selectedItems():
        ...
        print("Замітка для додавання теги не вибрана!")
